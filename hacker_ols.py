...
for OLS Regression & Hypothesis Testing
*********************************************************
"""
...
def rss_rmse(y_data: pd.Series, y_model: pd.Series, model_intercept: np.float, model_slope: np.float) -> tuple:
    ...
    print("Parameters a0={}, a1={} yield RSS={:0.2f}".format(model_intercept, model_slope, RSS))
...
def least_squares_np(x: pd.Series, y: pd.Series) -> tuple:
    """
    Purpose:
        Perform Ordinary Least Squares Linear Regression via numpy.
...
def conf_int95(np_array: np.ndarray) -> np.ndarray:
    ...
    Args:
        np_array (np.ndarray): explanatory variable
        y (pd.Series): dependent variable
        size (default=1): number of bootstrap samples to generate
...
def plot_hist_expected_ci(data: np.ndarray, distribution_mean: np.float, ci: np.ndarray, plt_title: str,
                          data_label: str, bins=50, kde=True) -> None:
    """
    Purpose:
        Compute the mean and standard deviation for a distribution.
    ...
    sns.distplot(data, bins=bins, kde=kde, hist_kws=dict(alpha=0.5, color='#00bfff', histtype='bar', ec='black'),
                 color='red')
    plt.title(plt_title + ' \n')
    plt.xlabel(data_label)
    # axis.legend(loc='best')

    # Add custom axis vertical lines for expected value and 95% confidence interval
    axis.axvline(distribution_mean, label='Expected Value', color='black')
    axis.axvline(ci[0], label=' 5th', color='red')
    axis.axvline(ci[1], label='95th', color='red')
...
def rss_minima_viz(x: pd.Series, y: pd.Series, slope_ci95: np.ndarray, intercept_ci95: np.ndarray,
    ...
    plt.title('Minimum RSS = {:0.2f} \n came from Slope = {:0.2f}'.format(df[df['RSS'] == best_rss]['RSS'].values[0],
                                                                          df[df['RSS'] == best_rss][
...
def plot_effect_size(test_statistic_dist: np.ndarray, effect_size: np.float, plt_title: str, data_label: str,
                     bins=10) -> None:
    """
    Purpose:
        Plot a histogram of the test statistic distribution with a vertical line for the effect size.

    Args:
        test_statistic_dist (np.ndarray): test statistic distribution
        effect_size (np.float): effect size
        plt_title (str): title of Chart
        data_label (str): x-axis title
        bins (default=10): number of bins for Histogram
    ...
    # Plot histogram of test statistic and a vertical line for the effect size
    plt.hist(test_statistic_dist, bins=bins, histtype='bar', ec='black', color="#00bfff",
             label='shuffled t-stat')
    plt.axvline(effect_size, label='effect_size', color='black')
    ...
    # Plot with a tight layout and show the chart
    plt.tight_layout()
    plt.show()
